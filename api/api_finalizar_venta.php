...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'get_sucursales') {
        ...
        // Llamamos a la función que ya pusiste en funciones_db.php
        echo json_encode(obtenerSucursalesPorUsuario($pdo, $usuario_id));
        exit;
    }

    // POST: Guardar los 3 formularios y el ID de sucursal
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $id_cot = (int)($_POST['id_cotizacion'] ?? 0);
        if ($id_cot === 0) exit(json_encode(['status' => 'error', 'message' => 'ID inválido.']));

        $sucursal_id = (int)($_POST['Sucursal_id'] ?? 0); // Capturamos la sucursal seleccionada
        ...
        // Pasamos el sucursal_id a la función
        formalizarVenta($pdo, $id_cot, $fiscal, $cert, $envio, $sucursal_id);
