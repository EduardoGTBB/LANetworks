...
define('SMTP_USER', 'hola@avisos.lupware.com');
...
try{
    $pdo = new PDO($CPDO, DB_USER, DB_PASS, $options);
}catch(PDOException $e){ // 🛡️ Cambio 1: Capturamos específicamente PDOException
    // Error grave de conexión. No podemos continuar.
    http_response_code(500); 
    ...
    // Mensaje genérico para el cliente/consola
    echo json_encode([
        'success' => false,
        'message' => 'Error interno del servidor. No se pudo conectar a la base de datos.'
    ]);
    
    // 🛡️ Cambio 2: Detenemos la ejecución en seco para evitar errores en cascada
    exit; 
}
?>