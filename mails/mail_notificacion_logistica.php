...
function enviarCorreoLogistica($destinatario, $nombre_cliente, $folio, $paqueteria, $numero_guia, $fecha_envio, $fecha_estimada = 'No especificada') {
    ...
    try {
        ...
        // Si no capturaron fecha estimada, ponemos un texto por defecto
        $fecha_est_segura = empty($fecha_estimada) ? 'Pendiente' : htmlspecialchars($fecha_estimada, ENT_QUOTES, 'UTF-8');
        ...
        $mail->Body = <<<HTML
            <div style='font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eaeaea; padding: 20px; border-radius: 8px;'>
                ...
                <h2 style='color: #0d6efd;'>Estimada/o {$nombre_seguro},</h2>
                ...
                <div style='background-color: #f8f9fa; border-left: 4px solid #0d6efd; padding: 15px; margin: 20px 0;'>
                    ...
                    <p style='margin: 5px 0;'><strong>⏳ Estimada de recepción:</strong> <span style='color: #198754; font-weight:bold;'>{$fecha_est_segura}</span></p>
