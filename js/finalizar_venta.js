$(document).ready(function () {
    ...
    // ✨ FUNCIÓN MAESTRA DE SEGURIDAD UX PARA DESBLOQUEAR INPUTS
    function toggleInputsEnvio(desbloquear) {
        let $inputs = $('#envio_gral_calle, #envio_gral_entre, #envio_gral_y, #envio_gral_colonia, #envio_gral_localidad, #envio_gral_municipio, #envio_gral_estado, #envio_gral_cp');
        if (desbloquear) {
            // Arrancamos el atributo físicamente del DOM
            $inputs.removeAttr('readonly').removeClass('bg-light');
        } else {
            $inputs.attr('readonly', 'readonly').addClass('bg-light');
        }
    }
        ...
        success: function (res) {
            if (res.status === 'success' && res.data) {
                ...
                if (d.detalles && d.detalles.length > 0) {
                    ...
                    let options = '<option value="">Seleccionar Domicilio...</option>';
                    options += '<option value="MANUAL" class="fw-bold text-primary">✏️ Escribir manualmente...</option>';
                    ...
                    options += '<option value="FISCAL" class="fw-bold text-dark">🚚 Ocupar Sin Sucursal (Fiscal)</option>';
                    if (isSucursalUnica) {
                    ...
                    // ✨ 1. BLOQUEO INICIAL SEGURO
                    toggleInputsEnvio(false);

                    // ✨ 2. EVENTO DEL SELECTOR
                    $('#selector_contacto_gral').off('change').on('change', function () {
                        ...
                        if (val === '') {
                            ...
                            // Escribir manualmente: Limpiar y DESBLOQUEAR
                            $('#envio_gral_calle, #envio_gral_entre, #envio_gral_y, #envio_gral_colonia, #envio_gral_localidad, #envio_gral_municipio, #envio_gral_estado, #envio_gral_cp').val('');
                            toggleInputsEnvio(true);
                            $('#envio_gral_calle').focus();
                        } else {
                            // Plaza existente: Llenar y DESBLOQUEAR
                            toggleInputsEnvio(true);
                    ...
                    // ✨ 3. PRELLENADO SI LA BD YA TENÍA DIRECCIÓN GUARDADA
                    if (isSucursalUnica) {
                        ...
                        if (tieneEnvioGuardado) {
                            ...
                            toggleInputsEnvio(true); // DESBLOQUEO SEGURO
                            ...
                            if ($('#selector_contacto_gral').length > 0) {
                                ...
                                if (indexPlaza !== -1) {
                                    ...
                                    $('#selector_contacto_gral').val('MANUAL');
                                }
                            }
                        }
                    ...
                    // ✨ BARRIDO FINAL DE SEGURIDAD (Garantiza que si hay texto, se puedan editar)
                    setTimeout(() => {
                        if ($('#envio_gral_calle').val().trim() !== '' || $('#selector_contacto_gral').val() !== '') {
                            toggleInputsEnvio(true);
                        }
                    }, 100);

                    // Alerta de Equipos sin dirección
                    ...
                    if (numSinDir > 0 && (vieneDeEdicion || esEdicionIncomplete)) {
                        let msjExtra = isSucursalUnica
                            ? "El sistema ha pre-llenado la información en base a los otros equipos. Por favor verifícala y <strong>da clic en Guardar y Finalizar</strong> para registrar los equipos nuevos."
                            : "Por favor, revisa la información precargada o asigna manualmente la dirección a los equipos faltantes y haz clic en <strong>Guardar y Finalizar</strong>.";
    ...
    $('#envio_gral_calle, #envio_gral_entre, #envio_gral_y, #envio_gral_colonia, #envio_gral_localidad, #envio_gral_municipio, #envio_gral_estado, #envio_gral_cp').on('input', function () {
        ...
        if (!isSucursalUnica && $('#selector_contacto_gral').val() !== "MANUAL") $('#selector_contacto_gral').val('MANUAL');
    });
        ...
        success: function (res) {
            if (res.status === 'success' && res.data) {
                ...
                if (d.detalles && d.detalles.length > 0) {
                    ...
                    // BLOQUEO INICIAL DE SEGURIDAD UX
                    $('#envio_gral_calle, #envio_gral_entre, #envio_gral_y, #envio_gral_colonia, #envio_gral_localidad, #envio_gral_municipio, #envio_gral_estado, #envio_gral_cp').prop('readonly', true).addClass('bg-light');
                    ...
                    * $('#selector_contacto_gral').off('change').on('change', function () {
                    ...
                    // ✨ EVENTO DE CAMBIO DEL SELECTOR (EL "MÚSCULO" UX)
                    $('#selector_contacto_gral').off('change').on('change', function () {
                        let val = $(this).val();
                        let $inputs = $('#envio_gral_calle, #envio_gral_entre, #envio_gral_y, #envio_gral_colonia, #envio_gral_localidad, #envio_gral_municipio, #envio_gral_estado, #envio_gral_cp');

                        if (val === '') {
                            // Opción por defecto: Limpiar y bloquear
                            $inputs.val('').prop('readonly', true).addClass('bg-light');
                        } else if (val === 'MANUAL') {
                            // Escribir manualmente: Limpiar y DESBLOQUEAR
                            $inputs.val('').prop('readonly', false).removeClass('bg-light');
                            $('#envio_gral_calle').focus(); // Dar foco al primer input
                        } else {
                            // Si eligió Certificado, Fiscal o una Plaza: Llenar y DESBLOQUEAR para que pueda editar
